...
import com.example.Fuba_BE.dto.payment.*;
...
public class MomoPaymentServiceImpl implements MomoPaymentService {
    ...
    public PaymentResponse createPayment(Integer bookingId) {
        ...
        try {
            ...
            log.info("MoMo response: resultCode={}, message={}", momoResponse.getResultCode(), momoResponse.getMessage());
    ...
    public boolean handleIpnCallback(MomoIpnRequest ipnRequest) {
        ...
        // 2. Find booking by order ID (booking code) WITH PESSIMISTIC LOCK to prevent race conditions
        ...
        // 2.6. Reject if booking already expired (should not resurrect expired bookings)
        ...
        if (ipnRequest.getResultCode() == 0) {
            ...
            for (Ticket ticket : tickets) {
                ...
                if (!"Held".equals(seat.getStatus())) {
                    log.error("❌ Seat {} status is {} (expected Held) for booking {}. Payment rejected to prevent double booking.",
