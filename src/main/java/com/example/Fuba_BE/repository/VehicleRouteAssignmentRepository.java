...
public interface VehicleRouteAssignmentRepository extends JpaRepository<VehicleRouteAssignment, Integer> {
        ...
        */
       @Override
       @Query(value = "SELECT vra FROM VehicleRouteAssignment vra " +
                     "LEFT JOIN FETCH vra.vehicle v " +
                     "LEFT JOIN FETCH v.vehicleType vt " +
                     "LEFT JOIN FETCH vra.route r " +
                     "LEFT JOIN FETCH r.origin " +
                     "LEFT JOIN FETCH r.destination", countQuery = "SELECT COUNT(vra) FROM VehicleRouteAssignment vra")
       Page<VehicleRouteAssignment> findAll(Pageable pageable);
       ...
       /**
        * Find all assignments by vehicle with JOIN FETCH
        */
       @Query("SELECT vra FROM VehicleRouteAssignment vra " +
                     "LEFT JOIN FETCH vra.vehicle v " +
                     "LEFT JOIN FETCH v.vehicleType vt " +
                     ...
                     "WHERE vra.vehicle.vehicleId = :vehicleId " +
                     "ORDER BY vra.priority ASC")
       List<VehicleRouteAssignment> findByVehicleVehicleId(@Param("vehicleId") Integer vehicleId);
       ...
       @Query("SELECT vra FROM VehicleRouteAssignment vra " +
                     ...
                     "WHERE vra.route.routeId = :routeId " +
                     "ORDER BY vra.priority ASC")
       List<VehicleRouteAssignment> findByRouteRouteIdOrderByPriorityAsc(@Param("routeId") Integer routeId);
       ...
       @Query("SELECT vra FROM VehicleRouteAssignment vra " +
                     ...
                     "LEFT JOIN FETCH r.origin " +
                     "LEFT JOIN FETCH r.destination " +
                     "WHERE vra.assignmentId = :id")
       Optional<VehicleRouteAssignment> findByIdWithRelations(@Param("id") Integer id);
}
