...
public interface BookingRepository extends JpaRepository<Booking, Integer> {
    ...
    /**
     * Find booking by booking code with full trip details (eager loading)
     * Includes: trip, route, origin, destination, driver, driver.user, vehicle,
     * vehicleType
     */
    ...
    /**
     * Find booking by ticket code with full trip details (eager loading)
     * Note: This uses a subquery since Booking doesn't have a tickets collection
     */
    @Query("SELECT b FROM Booking b " +
            "LEFT JOIN FETCH b.trip t " +
            "LEFT JOIN FETCH t.route r " +
            ...
            "LEFT JOIN FETCH t.vehicle v " +
            "LEFT JOIN FETCH v.vehicleType " +
            "WHERE b.bookingId = (SELECT tk.booking.bookingId FROM Ticket tk WHERE tk.ticketCode = :ticketCode)")
    Optional<Booking> findByTicketCodeWithDetails(@Param("ticketCode") String ticketCode);

    /**
     * Find booking by ID with full trip details (eager loading)
     */
    @Query("SELECT b FROM Booking b " +
            "LEFT JOIN FETCH b.trip t " +
            "LEFT JOIN FETCH t.route r " +
    ...
    /**
     * Find booking by booking code with pessimistic lock (for IPN concurrency
     * safety)
     */
    ...
    /**
     * Find bookings by customer with status filter (multiple statuses) and future
     * departure time
     */
    ...
    /**
     * Count bookings by customer with status filter (multiple statuses) and future
     * departure time
     */
    ...
    List<Booking> findByCustomerIdAndBookingStatus(@Param("customerId") Integer customerId,
     ...
     * CRITICAL: Use holdExpiry (not createdAt or updatedAt) to determine
     * expiration.
     * 
     * Logic:
